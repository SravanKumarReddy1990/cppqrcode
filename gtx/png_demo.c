            ...
            const char  *text)
{
    ...
    HPDF_Page_DrawImage (page, image, x, y, 80,
                    80);
...
char *select="SELECT * FROM products where manuf_status=1";
//printf(select);
    ...
    draw_image (pdf, "sravan.png", j*100, HPDF_Page_GetHeight (page) - ((k*100)+50), PQgetvalue(res, i, 1));
   /* draw_image (pdf, "sravan.png", 200, HPDF_Page_GetHeight (page) - 150,"8bit grayscale.");
    draw_image (pdf, "basn0g04.png", 300, HPDF_Page_GetHeight (page) - 150,
                "4bit grayscale.");
    draw_image (pdf, "basn0g08.png", 400, HPDF_Page_GetHeight (page) - 150,
                "8bit grayscale.");*/
char *update="update products set manuf_status=1 where p_id='";
char *updates=strcat(update,PQgetvalue(res, i, 1));
char *updatess=strcat(updates,"'");

//printf(update);
     PQexec(conn, updatess);    
...
k++;
  ...
  /*  draw_image (pdf, "sravan.png", 100, HPDF_Page_GetHeight (page) - 250,
                "8bit color.");
    draw_image (pdf, "sravan.png", 200, HPDF_Page_GetHeight (page) - 250,
                "16bit color.");
